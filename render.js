...
window.autoSmrskniPismoTymu = (containerSelector) => {
    const provedSmrsknuti = () => {
    ...
    // 🧠 SŇAŤKOVÁ KANCELÁŘ: Propojíme Font API s jakýmkoliv kontejnerem, na který ukážeš
    if (document.fonts) {
        document.fonts.ready.then(() => {
            requestAnimationFrame(provedSmrsknuti);
        });
    } else {
        provedSmrsknuti();
    }
        ...
        if (isEvaluated) {
            ...
            rightSideGroupHtml = `
                <div class="action-inputs">
                    <select id="tip-domaci-${matchId}" class="select-score" style="color: ${barvaDomaci};" data-saved="${existingTip ? existingTip.tip_domaci : ''}" onchange="window.handleUserScoreChange('${matchId}', ${match.isPlayoff || false})">
                        ${generujMožnosti(vybranyDomaci)}
                    </select>
                    <span class="select-divider">:</span>
                    <select id="tip-hoste-${matchId}" class="select-score" style="color: ${barvaHoste};" data-saved="${existingTip ? existingTip.tip_hoste : ''}" onchange="window.handleUserScoreChange('${matchId}', ${match.isPlayoff || false})">
                        ${generujMožnosti(vybranyHoste)}
                    </select>
                </div>
                <button class="btn-tip" @click="window.saveTip('${matchId}', '${leagueName}', event)">
...
window.showPlayerTipsModal = async (playerUid, nickname, leagueName) => {
    ...
    if (window.tipniToCache.histories[playerUid]) {
        ...
        serazeneZapasy.forEach(zap => {
            ...
            // Pokud zápas zrovna reálně běží, vstříkneme před skóre pulzující červenou tečku
        ...
        window.openGlobalUiModal(`Tipy hráče: ${nickname}`, listHtml);
                ...
                // 🧠 AUTOMATICKÉ DYNAMICKÉ SMRSKÁVÁNÍ NÁZVŮ TÝMŮ V MODÁLU HISTORIE TIPŮ
                requestAnimationFrame(() => {
                    const modalBody = document.querySelector('.spy-modal-overlay .spy-modal-body');
                    if (!modalBody) return;
                    
                    modalBody.querySelectorAll('.player-tips-table-row').forEach(row => {
                        const teamsEl = row.querySelector('div:first-child');
                        if (!teamsEl) return;
                        
                        let currentSize = 0.90; // Výchozí bezpečná velikost pro fluidní 1fr sloupec v modálu
                        teamsEl.style.fontSize = `${currentSize}rem`;
                        const dostupnaSirka = teamsEl.clientWidth;
                        
                        if (dostupnaSirka > 0) {
                            while (teamsEl.scrollWidth > dostupnaSirka && currentSize > 0.65) {
                                currentSize -= 0.02;
                                teamsEl.style.fontSize = `${currentSize}rem`;
                            }
                        }
